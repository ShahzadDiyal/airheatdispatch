  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: "Service Areas",
        item: `${SITE_CONFIG.domain}/service-areas`,
      },
    ],
  };
    ...
    <>
      ...
      <div className="bg-slate-50 min-h-screen py-12 lg:py-16 text-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="my-12 p-8 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <Link
              href="/hvac/texas"
              className="px-6 py-3.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs uppercase tracking-wider inline-flex items-center gap-2 shrink-0 shadow-md transition-colors"
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {TEXAS_CITIES_DATA.map((cityData) => (
                <div
                  ...
                  className="bg-white border border-slate-200 hover:border-blue-300 rounded-2xl p-6 space-y-4 shadow-sm hover:shadow-md transition-all group flex flex-col justify-between"
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-blue-600 flex items-center gap-1">
                        <MapPin className="w-3.5 h-3.5 text-blue-600" />
                        {cityData.cityName}, TX
                      </span>
                      <span className="text-[10px] px-2.5 py-0.5 rounded-md bg-slate-100 border border-slate-200 text-slate-600 font-medium">
                  ...
                  <div className="pt-4 border-t border-slate-100 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-1 text-[11px] text-slate-500 pt-1">
                      <Link
                        href={`/ac-repair/texas/${cityData.citySlug}`}
                        className="hover:text-blue-600 transition-colors truncate"
                      >
                        • AC Repair {cityData.cityName}
                      </Link>
                      <Link
                        href={`/furnace-repair/texas/${cityData.citySlug}`}
                        className="hover:text-blue-600 transition-colors truncate"
                      >
