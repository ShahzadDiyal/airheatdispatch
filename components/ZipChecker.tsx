...
import { MapPin, CheckCircle2, Clock, Phone, AlertCircle } from "lucide-react";
...
export default function ZipChecker() {
  ...
  return (
    <div className="bg-gradient-to-br from-slate-900 via-slate-900 to-sky-950/40 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl my-10 relative overflow-hidden">
      ...
      <form onSubmit={handleCheck} className="mt-6 flex flex-col sm:flex-row gap-3 max-w-xl">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white text-sm focus:outline-none focus:border-sky-500"
            required
          />
        </div>
        <button
          type="submit"
          className="px-6 py-3 rounded-xl bg-sky-500 hover:bg-sky-400 text-white font-bold text-xs uppercase tracking-wider transition-colors cursor-pointer shrink-0"
        >
          Check Availability
        </button>
      </form>
      ...
      {result === "available" && (
        <div className="mt-4 p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs sm:text-sm space-y-2 animate-in fade-in">
          ...
          <a
            ...
            rel="dofollow"
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-500 text-slate-950 font-black text-xs hover:bg-emerald-400 transition-colors mt-1"
          >
